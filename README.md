...
When system configuration permits, Base64UUID strings in URLs MAY retain the `$` character without percent-encoding. Otherwise, the `$` character MUST be replaced with a hyphen (`-`) in URLs. Implementations MUST explicitly declare which approach they support.
